...
const SPREADSHEET =
  "https://sheet.best/api/sheets/9d2ed0fb-4da1-461e-9999-854a29628684";
...
async function fetchSecciones() {
  const response = await fetch(SPREADSHEET + "/tabs/Secciones");
...
async function UpdateIntro() {
  ...
  await fetch(SPREADSHEET + "/tabs/Parrafos")
    .then((resp) => resp.json())
    .then((main) => {
...
async function UpdateCitas() {
  ...
  await fetch(SPREADSHEET + "/tabs/Citas")
    .then((resp) => resp.json())
    .then((citas) => {
      const r_citas = citas;
...
async function UpdateRecursos() {
  ...
  await fetch(SPREADSHEET + "/tabs/Recursos")
    .then((resp) => resp.json())
    .then((materiales) => {
